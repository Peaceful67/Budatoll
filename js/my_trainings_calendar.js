...
function btEventClicked(eventInfo) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            switch (response.status) { // done, deleted, deleted-waiting, waiting, confirmed, confirmed-waiting, delete-refused
                case 'done':
                ...
                case 'deleted-waiting':
                    ...
                    break;
                case 'delete-refused':
                    error_msg.html('Az edzésről már nem lehet lejelentkezni').show(1000).delay(2500).hide(1000);
                    break;
                case 'waiting':
...
function btMyTrainingMouseEnter(eventInfo) {
    ...
    if ($("#budatoll-trainings-editor").is(":hidden")) {
        ...
        training_info.addClass("is-open").fadeIn(budatoll_modal_speed);
